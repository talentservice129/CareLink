(function ($) {
  ...
  function smoothScroll() {
    $('a[href*="#"]:not([href="#"])').click(function (e) {
      ...
      let id = $(this).attr("href"),
        top = $(id).offset().top;
      $("body,html").animate(
        { scrollTop: top - header.outerHeight() - 32 },
        1000
      );
    });
  }
  ...
  function sliderBubblesPagination({ $el, imagesToLoad, activeIndex }) {
    let arrNext = [],
      ...
      count2 = homeSlider[0]
        .querySelector(".reviews-slider__container")
    ...
    for (n = 1; n <= maxCount; n++) {
      ...
      //       arrNext.push({
      //         id: differenceNext,
      //         url: imagesToLoad[differenceNext].src,
      //       });
      //     } else {
      //       if (i == countNext) {
      ...
      for (let j = -maxCount; j < imagesToLoad.length; j++) {
        ...
        if (j < 0 && j == countPrev) {
          arrPrev.push({
            id: differencePrev,
            url: imagesToLoad[differencePrev].src,
          });
        } else {
          if (j >= 0 && j == countPrev) {
    ...
    //   bubblesNext.innerHTML += `<li class="bubbles-item bubbles-item-next-${
    //     k + 1
    //   }"><img src="${arrNext[k].url}"></li>`;
    // }
    if (arrPrev.length) {
      bubblesPrev.innerHTML += `<li class="bubbles-item bubbles-item-prev-1"><img src="${arrPrev[0].url}"></li>`;
    }
  }
    ...
    for (let i = 0; i < slides.length; i++) {
      ...
      if (
        slides[i]
          ...
          .classList.contains("readMore-container")
      ) {
        if (arrayContainer.classList.contains("active")) {
          let close = slides[i]
            .querySelector(".btn-readMore")
            .getAttribute("data-close");
          slides[i]
            .querySelector(".readMore-container")
  ...
  $(".play-button-svg").click(function () {
    ...
    $videoContainer.html(`<iframe width="880" height="495" src="https://www.youtube.com/embed/M66U_DuMCS8"
                                        title="" frameBorder="0"
